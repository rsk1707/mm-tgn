#!/bin/bash
#SBATCH --job-name=mmtgn_cloth_film
...
#SBATCH --output=logs/train_cloth_sota_film_%j.out
#SBATCH --error=logs/train_cloth_sota_film_%j.err

# ==============================================================================
# MM-TGN Training: Amazon Cloth SOTA Features (FiLM Fusion)
# Experiment: Feature-wise Linear Modulation (text modulates image)
# ==============================================================================

echo "🚀 MM-TGN Training: Amazon Cloth SOTA (FiLM Fusion)"
echo "Date: $(date)"
echo "Node: $(hostname)"
echo ""
...
python train_mmtgn.py \
    ...
    --no-eval-ranking \
    --n-neg-eval 100 \
    --run-name "cloth_sota_film_$(date +%Y%m%d)"
