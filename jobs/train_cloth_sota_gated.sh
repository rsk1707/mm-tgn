#!/bin/bash
#SBATCH --job-name=mmtgn_cloth_gated
...
#SBATCH --output=logs/train_cloth_sota_gated_%j.out
#SBATCH --error=logs/train_cloth_sota_gated_%j.err

# ==============================================================================
# MM-TGN Training: Amazon Cloth SOTA Features (Gated Fusion)
# Experiment: Learned attention weights for modality combination
# ==============================================================================

echo "🚀 MM-TGN Training: Amazon Cloth SOTA (Gated Fusion)"
echo "Date: $(date)"
echo "Node: $(hostname)"
echo ""
...
python train_mmtgn.py \
    ...
    --no-eval-ranking \
    --n-neg-eval 100 \
    --run-name "cloth_sota_gated_$(date +%Y%m%d)"
