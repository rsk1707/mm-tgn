#!/bin/bash
# ==============================================================================
# Smart Submission Script for Amazon Cloth Experiments
# Submits all training jobs, then queues evaluation jobs with dependencies
# ==============================================================================

echo "🚀 Amazon Cloth: Submitting All Experiments"
echo "=============================================="
echo "Date: $(date)"
echo ""
...
chmod +x jobs/train_cloth_*.sh jobs/eval_cloth_*.sh
...
TRAIN_VANILLA_JID=$(sbatch --parsable jobs/train_cloth_vanilla.sh)
echo "   ✅ Vanilla training: Job $TRAIN_VANILLA_JID"

TRAIN_SOTA_JID=$(sbatch --parsable jobs/train_cloth_sota.sh)
echo "   ✅ SOTA MLP training: Job $TRAIN_SOTA_JID"

TRAIN_FILM_JID=$(sbatch --parsable jobs/train_cloth_sota_film.sh)
echo "   ✅ SOTA FiLM training: Job $TRAIN_FILM_JID"

TRAIN_GATED_JID=$(sbatch --parsable jobs/train_cloth_sota_gated.sh)
...
EVAL_VANILLA_JID=$(sbatch --parsable --dependency=afterok:$TRAIN_VANILLA_JID jobs/eval_cloth_vanilla.sh)
echo "   ✅ Vanilla eval: Job $EVAL_VANILLA_JID (depends on $TRAIN_VANILLA_JID)"

EVAL_SOTA_JID=$(sbatch --parsable --dependency=afterok:$TRAIN_SOTA_JID jobs/eval_cloth_sota.sh)
echo "   ✅ SOTA MLP eval: Job $EVAL_SOTA_JID (depends on $TRAIN_SOTA_JID)"

EVAL_FILM_JID=$(sbatch --parsable --dependency=afterok:$TRAIN_FILM_JID jobs/eval_cloth_sota_film.sh)
echo "   ✅ SOTA FiLM eval: Job $EVAL_FILM_JID (depends on $TRAIN_FILM_JID)"

EVAL_GATED_JID=$(sbatch --parsable --dependency=afterok:$TRAIN_GATED_JID jobs/eval_cloth_sota_gated.sh)
...
echo "Results in: checkpoints/cloth_*/results_full.json"
echo ""
