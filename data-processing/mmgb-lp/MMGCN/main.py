...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--PATH_weight_save", default=None, help="Writing weight filename."
    )
    ...
    for epoch in range(num_epoch):
        ...
        if torch.isnan(loss):
            with open(
                "./Data/" + data_path + "/result_{0}.txt".format(save_file), "a"
            ) as save_file:
                save_file.write(
        ...
        val_precision, val_recall, val_ndcg = full_vt(
            epoch, model, val_data, "Val", writer
        )
