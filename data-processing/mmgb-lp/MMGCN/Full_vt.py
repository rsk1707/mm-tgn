    ...
    with no_grad():
        ...
        if writer is not None:
            ...
            writer.add_histogram(
                prefix + "_user_visual_distribution",
                model.user_preferences[:, :44],
                epoch,
            )
            writer.add_histogram(
                prefix + "_user_acoustic_distribution",
                model.user_preferences[:, 44:-44],
                epoch,
            )
            writer.add_histogram(
                prefix + "_user_textual_distribution",
                model.user_preferences[:, -44:],
                epoch,
            )
