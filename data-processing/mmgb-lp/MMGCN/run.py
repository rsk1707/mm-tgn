...
def get_rank_2d(pred, n=0, descending=True):
    ...
    ranks = (
        torch.nonzero(sorted_indices == n_tensor, as_tuple=False)[:, 1] + 1
    )  # Adding 1 to make ranks start from 1
...
def train(dataloader, model, optimizer, batch_size, max_step=None):
    ...
    for idx, batch in enumerate(tqdm(dataloader)):
        ...
        optimizer.zero_grad()
        loss, model_loss, reg_loss, weight_loss, entropy_loss = model.loss(
    ...
    def objective(trial):
        ...
        for epoch in range(args.num_epoch):
            ...
            if (epoch + 1) % 5 == 0:
                with torch.no_grad():
                    representation = model.forward()
                    val_mrr, val_hit_1, val_hit_10 = batch_evaluate_model(
                        representation, val_triple_data, verbose=False
                    )
                    if val_mrr > val_max_mrr:
                        val_max_mrr = val_mrr
                    print(f"{lr}, {num_layers}")
                    print(
                        f"Epoch: {epoch}, MRR: {val_max_mrr}, Hit@1: {val_hit_1}, Hit@10: {val_hit_10}"
                    )
        return val_max_mrr
...
def main():
    ...
    parser.add_argument(
        "--PATH_weight_load", default=None, help="Loading weight filename."
    )
    ...
    parser.add_argument(
        "--has_entropy_loss", default="False", help="Has Cross Entropy loss."
    )
    ...
    parser.add_argument(
        "--PATH_best_weight_save", default=None, help="Writing weight filename."
    )
    ...
    train_edge = torch.concat(
        [
            edge_split["train"]["source_node"].reshape(-1, 1),
            edge_split["train"]["target_node"].reshape(-1, 1),
        ],
        dim=1,
    )
    train_edge.shape
    ...
    if args.feat_path is None:
        num_nodes, train_edge, src_tgt_dict, v_feat, a_feat, t_feat = data_load_old(
            train_edge, args.v_feat_path, args.t_feat_path
        )
    else:
        num_nodes, train_edge, src_tgt_dict, v_feat, a_feat, t_feat = data_load(
            train_edge, args.feat_path
        )
    ...
    for try_time in range(args.repeat_times):
        ...
        for epoch in range(num_epoch):
            ...
            with torch.no_grad():
                ...
                test_mrr, test_hit_1, test_hit_10 = batch_evaluate_model(
                    representation, test_triple_data, verbose=False
                )
