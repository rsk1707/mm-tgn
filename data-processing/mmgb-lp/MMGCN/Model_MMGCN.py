...
class GCN(torch.nn.Module):
    ...
    def forward(self, features, id_embedding):
        ...
        if self.num_layers >= 4:
            ...
            x_hat = (
                F.leaky_relu(self.linear_layer4(x)) + id_embedding
                if self.has_id
                else F.leaky_relu(self.linear_layer4(x))
            )  # equation 6
            x = (
                F.leaky_relu(self.g_layer4(torch.cat((h, x_hat), dim=1)))
                if self.concate
                else F.leaky_relu(self.g_layer4(h) + x_hat)
            )

        if self.num_layers >= 5:
            h = F.leaky_relu(self.conv_embed_5(x, self.edge_index))  # equation 1
            x_hat = (
                F.leaky_relu(self.linear_layer5(x)) + id_embedding
                if self.has_id
                else F.leaky_relu(self.linear_layer5(x))
            )  # equation 7
            x = (
                F.leaky_relu(self.g_layer5(torch.cat((h, x_hat), dim=1)))
                if self.concate
                else F.leaky_relu(self.g_layer5(h) + x_hat)
            )
...
class Net(torch.nn.Module):
    def __init__(
        ...
        self.id_embedding = nn.init.xavier_normal_(
            torch.rand((num_node, dim_x), requires_grad=True)
        ).cuda()
    ...
    def loss(self, src_tensor, tgt_tensor):
        ...
        reg_embedding_loss = (
            self.id_embedding[src_tensor] ** 2 + self.id_embedding[tgt_tensor] ** 2
        ).mean()
