...
def get_loaders(args, dataset, splits, directed):
    ...
    if args.model in {"ELPH", "BUDDY"}:
        train_dataset, val_dataset, test_dataset = get_hashed_train_val_test_datasets(
            dataset, train_data, val_data, test_data, args, directed
        )
    else:
        t0 = time.time()
        train_dataset, val_dataset, test_dataset = get_train_val_test_datasets(
            dataset, train_data, val_data, test_data, args
        )
...
def get_data(args):
    ...
    if dataset_name.startswith("ogbl"):
        ...
        use_lcc_flag = False
        dataset = CustomPygLinkPropPredDataset(
            root=f"{ROOT_DIR}/../../../../Patton/{dataset_name}", feat=args.feat
        )
    elif dataset_name.startswith("book"):
        use_lcc_flag = False
        dataset = CustomPygLinkPropPredDataset(
            root=f"{ROOT_DIR}/../../../../Patton/{dataset_name}",
            feat=args.feat,
            hard=False,
        )
    else:
    ...
    if (
        dataset_name.startswith("ogbl")
        or dataset_name.startswith("sports")
        or dataset_name.startswith("cloth")
        or dataset_name.startswith("book")
    ):  # use the built in splits
        data = dataset[0]
