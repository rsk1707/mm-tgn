...
def test(
    ...
    pos_train_pred, neg_train_pred, train_pred, train_true = test_func(
        model, train_loader, device, args, split="train"
    )
    print("get val predictions")
    pos_val_pred, neg_val_pred, val_pred, val_true = test_func(
        model, val_loader, device, args, split="val"
    )
    print("get test predictions")
    pos_test_pred, neg_test_pred, test_pred, test_true = test_func(
        model, test_loader, device, args, split="test"
    )

    if (
        args.dataset_name.startswith("sports")
        or args.dataset_name.startswith("cloth")
        or args.dataset_name.startswith("book")
    ):
...
def get_buddy_preds(model, loader, device, args, split=None):
    ...
    loader = DataLoader(
        range(len(links)), args.eval_batch_size, shuffle=False
    )  # eval batch size should be the largest that fits on GPU
    ...
    for batch_count, indices in enumerate(tqdm(loader)):
        ...
        if args.use_struct_feature:
            ...
            subgraph_features = torch.zeros(data.subgraph_features[indices].shape).to(
                device
            )
...
def get_elph_preds(model, loader, device, args, split=None):
    ...
    for batch_count, indices in enumerate(tqdm(loader)):
        ...
        batch_emb = None if emb is None else emb[curr_links].to(device)
        if args.use_struct_feature:
            subgraph_features = model.elph_hashes.get_subgraph_features(
                curr_links, hashes, cards
            ).to(device)
        else:
            subgraph_features = torch.zeros(data.subgraph_features[indices].shape).to(
                device
        ...
        batch_node_features = (
            None if node_features is None else node_features[curr_links]
        )
