...
def print_results_list(results_list):
    for idx, res in enumerate(results_list):
        print(
            f"repetition {idx}: test {res[0]:.2f}, val {res[1]:.2f}, train {res[2]:.2f}"
        )
...
def main(cfg: DictConfig):
    ...
    for rep in range(args.reps):
        ...
        if args.dataset_name.startswith(
            "ogbl"
        ):  # then this is one of the ogb link prediction datasets
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    to_print = (
                        f"Epoch: {epoch:02d}, Epoch: {epoch}, Loss: {loss:.4f}, Train: {100 * train_res:.2f}%, Valid: "
