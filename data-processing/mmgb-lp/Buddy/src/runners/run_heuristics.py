    ...
    for heuristic in [RA, CN, AA, PPR]:
        ...
        for rep in range(args.reps):
            ...
            A_train = ssp.csr_matrix(
                (train_weight, (train_edges[0], train_edges[1])),
                shape=(num_nodes, num_nodes),
            )
            A_test = ssp.csr_matrix(
                (test_weight, (test_edges[0], test_edges[1])),
                shape=(num_nodes, num_nodes),
            )
            ...
            if args.dataset_name == "ogbl-citation2":
                ...
                print(
                    f"evaluating {pos_test_pred.shape} positive samples and {neg_test_pred.shape} negative samples"
                )
        ...
        if args.reps > 1:
            train_acc_mean, val_acc_mean, test_acc_mean = (
                np.mean(results_list, axis=0) * 100
            )
            test_acc_std = np.sqrt(np.var(results_list, axis=0)[-1]) * 100
            wandb_results = {
                f"{heuristic.__name__}_test_mean": test_acc_mean,
            ...
            print(wandb_results)
        print(
            f"{heuristic.__name__} ran in {time.time() - t0:.1f} s for {args.reps} reps"
        )
    wandb.finish()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="the number of repetition of the experiment to run",
    )
    parser.add_argument(
        "--sample_size",
        type=int,
