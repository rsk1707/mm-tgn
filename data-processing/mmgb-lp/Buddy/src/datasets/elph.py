...
class HashDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            args.model != "ELPH"
        ):  # elph stores the hashes directly in the model class for message passing
            self.elph_hashes = ElphHashes(
                args
            )  # object for hash and subgraph feature operations
        ...
        if self.directed:  # make undirected graphs like citation2 directed
            ...
            self.edge_index, self.edge_weight = to_undirected(
                data.edge_index, self.edge_weight
            )
        else:
        ...
        self.degrees = torch.tensor(
            self.A.sum(axis=0, dtype=float), dtype=torch.float
        ).flatten()
        ...
        if (
            args.model == "ELPH"
        ):  # features propagated in the model instead of preprocessed
            self.x = data.x
        else:
            self.x = self._preprocess_node_features(
                data, self.edge_index, self.edge_weight, args.sign_k
            )
            # ELPH does hashing and feature prop on the fly
            # either set self.hashes or self.subgraph_features depending on cmd args
            self._preprocess_subgraph_features(
                self.edge_index.device, data.num_nodes, args.num_negs
            )
    ...
    def _generate_sign_features(self, data, edge_index, edge_weight, sign_k):
        ...
        if sign_k == 0:
            ...
            xs = torch_sparse.spmm(
                edge_index, edge_weight, data.x.shape[0], data.x.shape[0], data.x
            )
        else:
            xs = [data.x]
    ...
    def _read_subgraph_features(self, name, device):
        ...
        if self.cache_subgraph_features and os.path.exists(name):
            ...
            assert self.subgraph_features.shape[0] == len(self.links), (
                "subgraph features are inconsistent with the link object. Delete subgraph features file and regenerate"
            )
            retval = True
        return retval
    ...
    def _generate_file_names(self, num_negs):
        ...
        if num_negs == 1 or self.split != "train":
            ...
            subgraph_cache_name = (
                f"{self.root}{self.split}_negs{num_negs}{year_str}{end_str}"
            )
    ...
    def _preprocess_subgraph_features(self, device, num_nodes, num_negs=1):
        ...
        found_subgraph_features = self._read_subgraph_features(
            subgraph_cache_name, device
        )
        if not found_subgraph_features:
            ...
            if self.load_hashes and os.path.exists(hash_name):
                ...
                start_time = time()
                hashes, cards = self.elph_hashes.build_hash_tables(
                    num_nodes, self.edge_index
                )
            ...
            start_time = time()
            self.subgraph_features = self.elph_hashes.get_subgraph_features(
                self.links, hashes, cards, self.args.subgraph_feature_batch_size
            )
            # print("Preprocessed subgraph features in: {:.2f} seconds".format(time() - start_time))
            assert self.subgraph_features.shape[0] == len(self.links), (
                "subgraph features are a different shape link object. Delete subgraph features file and regenerate"
            )
            if self.cache_subgraph_features:
        ...
        if (
            not self.use_zero_one and self.subgraph_features is not None
        ):  # knock out the zero_one features (0,1) and (1,0)
            if self.max_hash_hops > 1:
                self.subgraph_features[:, [4, 5]] = 0
            if self.max_hash_hops == 3:
                self.subgraph_features[:, [11, 12]] = (
                    0  # also need to get rid of (0, 2) and (2, 0)
                )
    ...
    def get(self, idx):
        ...
        if self.args.use_struct_feature:
            ...
            subgraph_features = torch.zeros(
                self.max_hash_hops * (2 + self.max_hash_hops)
            )
        ...
        node_features = torch.cat(
            [self.x[src].unsqueeze(dim=0), self.x[dst].unsqueeze(dim=0)], dim=0
        )
        return subgraph_features, node_features, src_degree, dst_degree, RA, y


def get_hashed_train_val_test_datasets(
    dataset, train_data, val_data, test_data, args, directed=False
):
...
class HashedTrainEvalDataset(Dataset):
    ...
    def __init__(self, links, labels, subgraph_features, RA, dataset):
...
def make_train_eval_data(
    args, train_dataset, num_nodes, n_pos_samples=5000, negs_per_pos=1000
):
    """
    ...
    neg_sample = train_dataset.neg_edges[
        : n_pos_samples * negs_per_pos
    ]  # [num_neg_edges, 2]
    assert torch.all(
        torch.eq(pos_sample[:, 0].repeat_interleave(negs_per_pos), neg_sample[:, 0])
    ), (
        "negatives have different source nodes to positives. Delete train_eval_negative_samples_* and subgraph features and regenerate"
    )
    ...
    neg_sf = train_dataset.subgraph_features[
        n_pos_edges : n_pos_edges + len(neg_sample)
    ]
    ...
    train_eval_dataset = HashedTrainEvalDataset(
        links, labels, subgraph_features, RA_links, train_dataset
    )
