...
from src.utils import get_pos_neg_edges, get_src_dst_degree, neighbors
...
class SEALDynamicDataset(Dataset):
    ...
    def get(self, idx):
        ...
        y = self.labels[idx]
        src_degree, dst_degree = get_src_dst_degree(
            src, dst, self.A, self.max_nodes_per_hop
        )
        if self.sign:
            x = [self.data.x]
        ...
        data = construct_pyg_graph(
            *tmp, self.node_label, self.max_dist, src_degree, dst_degree
        )
