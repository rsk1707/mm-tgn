...
from torch.nn import BatchNorm1d as BN, Linear, Parameter
...
class GCNCustomConv(torch.nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, bias=True, propagate_features=False, **kwargs
    ):
...
class SAGE(torch.nn.Module):
    def __init__(
        self, in_channels, hidden_channels, out_channels, num_layers, dropout, residual
    ):
        ...
        for _ in range(num_layers - 2):
            self.convs.append(
                SAGEConv(hidden_channels, hidden_channels, root_weight=residual)
            )
...
class SIGNEmbedding(SIGNBaseClass):
    def __init__(self, in_channels, hidden_channels, out_channels, K, dropout):
        super(SIGNEmbedding, self).__init__(
            in_channels, hidden_channels, out_channels, K, dropout
        )
...
class SIGN(SIGNBaseClass):
    ...
    def forward(self, xs):
        ...
        for lin, bn, x in zip(self.lins, self.bns, xs):
            ...
            h = torch.cat(
                (bn(h[:, 0, :]).unsqueeze(1), bn(h[:, 1, :]).unsqueeze(1)), dim=1
            )
            h = F.relu(h)
            h = F.dropout(h, p=self.dropout, training=self.training)
            hs.append(h)
