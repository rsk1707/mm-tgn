...
class KGEModel(nn.Module):
    ...
    def score_sample(self, sample, mode="single"):
        if mode == "single":
            ...
            head = (
                torch.index_select(self.entity_embedding, dim=0, index=sample[0, :])
                .unsqueeze(1)
                .to(sample.device)
            )
            ...
            relation = (
                torch.index_select(
                    ...
                    dim=0,
                    index=torch.zeros(
                        sample.shape[1], dtype=torch.int32, device=sample.device
                    ),
                )
                .unsqueeze(1)
                .to(sample.device)
            ...
            tail = (
                torch.index_select(self.entity_embedding, dim=0, index=sample[1, :])
                .unsqueeze(1)
                .to(sample.device)
            )
            ...
            tail = (
                torch.index_select(self.entity_embedding, dim=0, index=tail_part[:, 2])
                .unsqueeze(1)
                .to(sample.device)
            )
            ...
            head = (
                torch.index_select(self.entity_embedding, dim=0, index=head_part[:, 0])
                .unsqueeze(1)
                .to(sample.device)
            )
