...
class LinkPredictor(torch.nn.Module):
    def __init__(self, args, use_embedding=False):
        ...
        if args.use_feature:
            self.lin_feat = Linear(args.hidden_channels, args.hidden_channels)
        ...
        if self.use_embedding:
            self.lin_emb = Linear(args.hidden_channels, args.hidden_channels)
...
class ELPH(torch.nn.Module):
    ...
    def __init__(self, args, num_features, node_embedding=None):
        ...
        self._convolution_builder(
            num_features, args.hidden_channels, args
        )  # build the convolutions for features and embs
        ...
        if self.sign_k != 0 and self.propagate_embeddings:
            ...
            self.sign_embedding = SIGNEmbedding(
    ...
    def _convolution_builder(self, num_features, hidden_channels, args):
        ...
        if self.node_embedding is not None:
            ...
            for _ in range(
                self.num_layers
            ):  # assuming the embedding has hidden_channels dims
    ...
    def propagate_embeddings_func(self, edge_index):
        ...
        return self.sign_embedding(
            self.node_embedding.weight, gcn_edge_index, num_nodes
        )
    ...
    def forward(self, x, edge_index):
        ...
        @return:
        """
        hash_edge_index, _ = add_self_loops(
            edge_index
        )  # unnormalised, but with self-loops
        ...
        if self.init_hashes == None:
            self.init_hashes = self.elph_hashes.initialise_minhash(num_nodes).to(
                x.device
            )
        ...
        for k in range(self.num_layers + 1):
            ...
            node_hashings_table[k] = {
                "hll": torch.zeros(
                    (num_nodes, self.hll_size),
                    dtype=torch.int8,
                    device=edge_index.device,
                ),
                "minhash": torch.zeros(
                    (num_nodes, self.num_perm),
                    dtype=torch.int64,
                    device=edge_index.device,
                ),
            }
            ...
            if k == 0:
                ...
                if self.feature_prop in {
                    "residual",
                    "cat",
                }:  # need to get features to the hidden dim
                    x = self._encode_features(x)

            else:
                node_hashings_table[k]["hll"] = self.elph_hashes.hll_prop(
                    node_hashings_table[k - 1]["hll"], hash_edge_index
                )
                node_hashings_table[k]["minhash"] = self.elph_hashes.minhash_prop(
                    node_hashings_table[k - 1]["minhash"], hash_edge_index
                )
                cards[:, k - 1] = self.elph_hashes.hll_count(
...
class BUDDY(torch.nn.Module):
    ...
    def __init__(self, args, num_features=None, node_embedding=None):
        ...
        self.dim = (
            num_labelling_features * 2
            if self.append_normalised
            else num_labelling_features
        )
        ...
        if args.use_feature:
            ...
            self.lin_out = Linear(args.hidden_channels, args.hidden_channels)
        hidden_channels = (
            self.dim + args.hidden_channels if self.use_feature else self.dim
        )
        if self.node_embedding is not None:
            self.lin_emb = Linear(args.hidden_channels, args.hidden_channels)
            self.lin_emb_out = Linear(args.hidden_channels, args.hidden_channels)
    ...
    def forward(
        self, sf, node_features, src_degree=None, dst_degree=None, RA=None, emb=None
    ):
        """
