...
class ElphHashes(object):
    ...
    def __init__(self, args):
        assert args.max_hash_hops in {1, 2, 3}, (
            f"hashing is not implemented for {args.max_hash_hops} hops"
        )
        self.max_hops = args.max_hash_hops
        self.floor_sf = (
            args.floor_sf
        )  # if true set minimum sf to 0 (they're counts, so it should be)
        # minhash params
        ...
        self.bias_vector = torch.tensor(
            hyperloglog_const._bias[self.p - 4], dtype=torch.float
        )
        self.estimate_vector = torch.tensor(
            hyperloglog_const._raw_estimate[self.p - 4], dtype=torch.float
        )
    ...
    def initialise_minhash(self, n_nodes):
        ...
        phv = np.bitwise_and(
            (a * np.expand_dims(hv, 1) + b) % self._mersenne_prime, self._max_minhash
        )
        hv = np.minimum(phv, init_hv)
        return torch.tensor(
            hv, dtype=torch.int64
        )  # this conversion should be ok as self._max_minhash < max(int64)

    def initialise_hll(self, n_nodes):
        regs = np.zeros(
            (n_nodes, self.m), dtype=np.int8
        )  # the registers to store binary values
        hv = hash_array(
            np.arange(1, n_nodes + 1)
        )  # this function hashes 0 -> 0, so avoid
        ...
        bits = hv >> self.p
        # Update the register
        ranks = self._get_hll_rank(
            bits
        )  # get the number of leading zeros in each element of bits
        regs[np.arange(n_nodes), reg_index] = np.maximum(
            regs[np.arange(n_nodes), reg_index], ranks
        )
        return torch.tensor(
            regs, dtype=torch.int8
        )  # int8 is fine as we're storing leading zeros in 64 bit numbers
    ...
    def build_hash_tables(self, num_nodes, edge_index):
        ...
        for k in range(self.max_hops + 1):
            ...
            node_hashings_table[k] = {
                "hll": torch.zeros((num_nodes, self.hll_size), dtype=torch.int8),
    ...
    def _estimate_bias(self, e):
        ...
        nearest_neighbors = torch.argsort(
            (e.unsqueeze(-1) - self.estimate_vector.to(e.device)) ** 2
        )[:, :6]
    ...
    def hll_count(self, regs):
        ...
        e = (self.alpha * self.m**2) / torch.sum(
            2.0 ** (-regs[estimate_indices]), dim=1
        )
    ...
    def jaccard(self, src, dst):
        ...
        if src.shape != dst.shape:
            raise ValueError(
                "source and destination hash value shapes must be the same"
            )
    ...
    def get_subgraph_features(self, links, hash_table, cards, batch_size=11000000):
        ...
        link_loader = DataLoader(
            range(links.size(0)), batch_size, shuffle=False, num_workers=0
        )
        all_features = []
        for batch in tqdm(link_loader):
            intersections = self._get_intersections(links[batch], hash_table)
            cards1, cards2 = (
                cards.to(links.device)[links[batch, 0]],
                cards.to(links.device)[links[batch, 1]],
            )
            features = torch.zeros(
                (len(batch), self.max_hops * (self.max_hops + 2)),
                dtype=float,
                device=links.device,
            )
            ...
            if self.max_hops == 1:
                ...
                features[:, 4] = cards2[:, 0] - torch.sum(
                    features[:, 0:2], dim=1
                )  # (0, 1)
                features[:, 5] = (
                    cards1[:, 0] - features[:, 0] - features[:, 2]
                )  # (1, 0)
                features[:, 6] = cards2[:, 1] - torch.sum(
                    features[:, 0:5], dim=1
                )  # (0, 2)
                features[:, 7] = (
                    cards1[:, 1]
                    - features[:, 0]
                    - torch.sum(features[:, 0:4], dim=1)
                    - features[:, 5]
                )  # (2, 0)
                ...
                features[:, 4] = (
                    intersections[(3, 1)] - features[:, 0] - features[:, 1]
                )  # (3,1)
                features[:, 5] = (
                    intersections[(1, 3)] - features[:, 0] - features[:, 2]
                )  # (1, 3)
                features[:, 6] = (
                    intersections[(3, 2)]
                    - torch.sum(features[:, 0:4], dim=1)
                ...
                features[:, 8] = intersections[(3, 3)] - torch.sum(
                    features[:, 0:8], dim=1
                )  # (3,3)
                features[:, 9] = (
                    cards2[:, 0] - features[:, 0] - features[:, 1] - features[:, 4]
                )  # (0, 1)
                features[:, 10] = (
                    cards1[:, 0] - features[:, 0] - features[:, 2] - features[:, 5]
                )  # (1, 0)
                features[:, 11] = (
                    cards2[:, 1]
                    - torch.sum(features[:, 0:5], dim=1)
                    - features[:, 6]
                    - features[:, 9]
                )  # (0, 2)
                features[:, 12] = (
            ...
            if not self.use_zero_one:
                if (
                    self.max_hops == 2
                ):  # for two hops any positive edge that's dist 1 from u must be dist 2 from v etc.
                    features[:, 4] = 0
                    features[:, 5] = 0
                elif (
                    self.max_hops == 3
                ):  # in addition for three hops 0,2 is impossible for positive edges
                    features[:, 4] = 0
                    features[:, 5] = 0
                    features[:, 11] = 0
                    features[:, 12] = 0
            if (
                self.floor_sf
            ):  # should be more accurate, but in practice makes no difference
