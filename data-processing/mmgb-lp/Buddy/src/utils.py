...
def get_same_source_negs(num_nodes, num_negs_per_pos, pos_edge):
    ...
    """
    print(
        f"generating {num_negs_per_pos} single source negatives for each positive source node"
    )
    dst_neg = torch.randint(
        0, num_nodes, (1, pos_edge.size(1) * num_negs_per_pos), dtype=torch.long
    )
...
def get_src_dst_degree(src, dst, A, max_nodes):
    ...
    """
    src_degree = (
        A[src].sum() if (max_nodes is None or A[src].sum() <= max_nodes) else max_nodes
    )
    dst_degree = (
        A[dst].sum() if (max_nodes is None or A[src].sum() <= max_nodes) else max_nodes
    )
