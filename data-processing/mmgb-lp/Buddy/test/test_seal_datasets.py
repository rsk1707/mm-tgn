...
class SEALDatasetTests(unittest.TestCase):
    ...
    def test_k_hop_subgraph(self):
        ...
        src, dst = 0, 1
        nodes, subgraph, dists, node_features, y = k_hop_subgraph(
            src,
            dst,
            num_hops,
            self.A,
        ...
        data = construct_pyg_graph(
            nodes, subgraph, dists, node_features, y, node_label="drnl"
        )
        self.assertTrue(
            data.edge_index.shape[1] == 4
        )  # 3 edges minus the 0-1 edge and undirected
        src, dst = 1, 2
        nodes, subgraph, dists, node_features, y = k_hop_subgraph(
            src,
            dst,
        ...
        self.assertTrue(
            data.edge_index.shape[1] == 6
        )  # 4 edges minus 1->2 and 2->1, but undirected
        src, dst = 2, 1
        nodes, subgraph, dists, node_features, y = k_hop_subgraph(
            src,
            dst,
    ...
    def test_seal_dynamic_dataset(self):
        ...
        self.assertTrue(
            len(labels) == self.test_edges.size(1) + self.neg_test_edges.size(1)
        )  # one prediction for each pos and neg edges
        self.assertTrue(
            sum(labels) == self.test_edges.size(1)
        )  # pos edges are labelled 1 and neg edges labelled 0
    ...
    def test_seal_dataset(self):
        ...
        self.assertTrue(
            labels.shape[0] == self.test_edges.size(1) + self.neg_test_edges.size(1)
        )  # one prediction for each pos and neg edges
        self.assertTrue(
            sum(labels) == self.test_edges.size(1)
        )  # pos edges are labelled 1 and neg edges labelled 0
