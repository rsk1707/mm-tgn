...
class ELPHTests(unittest.TestCase):
    ...
    def test_propagate(self):
        ...
        self.assertTrue(
            hashes[0]["minhash"].shape == (self.n_nodes, args.minhash_num_perm)
        )
        ...
        x, hashes, cards = gnn(self.x, self.edge_index)
        self.assertTrue(
            gnn.node_embedding.weight.shape == (self.n_nodes, args.hidden_channels)
        )
    ...
    def test_get_gnn_preds(self):
        ...
        dl = DataLoader(hdd, batch_size=1, shuffle=False, num_workers=1)
        pos_pred, neg_pred, pred, labels = get_elph_preds(
            gnn, dl, self.x.device, args, split="test"
        )
