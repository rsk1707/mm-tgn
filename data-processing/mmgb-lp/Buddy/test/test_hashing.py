...
from src.hashing import LABEL_LOOKUP, ElphHashes
...
class HashingTests(unittest.TestCase):
    ...
    def test_find_neighbourhood_cardinality(self):
        ...
        n1, n2 = (
            len(neighbors1) + 1,
            len(neighbors2) + 1,
        )  # node is counted by find_neighbourhood_cardinality
    ...
    def test_get_features(self):
        ...
        tmp_features = eh.get_subgraph_features(
            torch.tensor([[node1, node2]]), hash_tables, cards
        ).squeeze()
        ...
        feat33 = int33.difference(
            int11 | feat21 | feat12 | feat22 | feat31 | feat13 | feat23 | feat32
        )
    ...
    def test_hll_p(self):
        ...
        eh = ElphHashes(self.args)
        hash_table, cards = eh.build_hash_tables(self.n_nodes, self.edge_index)
        intersections = eh._get_intersections(
            torch.tensor([[node1, node2]]), hash_table
        )
    ...
    def test_refine_hll_count_estimate(self):
        ...
        n_links = 10
        estimates = (
            torch.rand(n_links) + 5 * eh.m - 0.5
        )  # make sure we have estimates above and below the threshold
    ...
    def test_jaccard(self):
        ...
        neighbors11 = neighbors([node1], self.A).union({node1})
        neighbors21 = neighbors([node2], self.A).union({node2})
        jaccard = len(neighbors11.intersection(neighbors21)) / len(
            neighbors11.union(neighbors21)
        )
    ...
    def test_initialise_hll(self):
        ...
        self.assertTrue(
            np.array_equal(np.count_nonzero(regs, axis=1), np.ones(self.n_nodes))
        )
        self.assertTrue(np.amin(regs) >= 0)
        self.assertTrue(np.amax(regs) <= eh.max_rank)
        for node in range(self.n_nodes):
            self.assertTrue(
                np.isclose(eh.hll_count(torch.tensor(regs[node])).item(), 1, atol=0.1)
            )
    ...
    def test_subgraph_features(self):
        ...
        self.assertTrue(
            sf.shape
            == (n_links, self.args.max_hash_hops * (self.args.max_hash_hops + 2))
        )
        self.args.max_hash_hops = 3
        eh = ElphHashes(self.args)
        hashes, cards = eh.build_hash_tables(self.n_nodes, self.edge_index)
        ...
        features = {
            eh.label_lookup[idx]: val for idx, val in enumerate(tmp_features.flatten())
        }
        # test (1,1) features
        int11 = neighbors11.intersection(neighbors21)
        self.assertTrue(isclose(len(int11), features[(1, 1)], abs_tol=1))
