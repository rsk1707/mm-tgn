...
class DataTests(unittest.TestCase):
    def setUp(self):
        ...
        self.valid = {
            "edge": torch.randint(0, edges, size=(edges, 2)),
    ...
    def test_make_obg_supervision_edges(self):
        labels, edges = make_obg_supervision_edges(
            self.split_edge, "test", neg_edges=None
        )
    ...
    def test_get_ogb_data(self):
        ...
        self.assertTrue(
            torch.all(torch.eq(splits["train"].edge_index, splits["valid"].edge_index))
        )
    ...
    def test_use_val_edges(self):
        ...
        self.assertTrue(
            not is_undirected(val_edges)
        )  # no reason for supervision edges to be directed
    ...
    def test_get_ogb_train_negs(self):
        ...
        train_negs = get_ogb_train_negs(
            split_edge, ei, num_nodes, dataset_name="ogbl-citation2"
        )
        ...
        train_negs = get_ogb_train_negs(
            split_edge, ei, num_nodes, num_negs=2, dataset_name="ogbl-citation2"
        )
    ...
    def test_sample_hard_negatives(self):
        ...
        reg_negs = negative_sampling(
            edge_index, num_nodes=len(G.nodes), num_neg_samples=10
        )
