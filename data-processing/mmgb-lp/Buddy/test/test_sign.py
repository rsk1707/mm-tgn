...
class SIGNTests(unittest.TestCase):
    ...
    def test_generate_sign_features(self):
        ...
        root = "."
        data = Data(
            torch.rand(self.n_nodes, n_features), self.edge_index, self.edge_weight
        )
        elph_dataset = HashDataset(
            root, split, data, self.test_edges, self.neg_test_edges, args
        )
        x = elph_dataset._generate_sign_features(
            data, self.edge_index, self.edge_weight, sign_k
        )
    ...
    def test_sign_forward(self):
        ...
        x = torch.rand(
            (batch_size, 2, hidden_channels * (K + 1))
        )  # the original features + K propagated features
