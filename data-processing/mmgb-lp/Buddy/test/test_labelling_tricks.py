...
from src.labelling_tricks import (
    de_node_labeling,
    de_plus_node_labeling,
    drnl_hash_function,
    drnl_node_labeling,
    get_drnl_lookup,
)
...
class LabelingTrickTests(unittest.TestCase):
    def setUp(self):
        ...
        self.edge_index = torch.tensor(
            np.concatenate([square1, square2, bridge], axis=1), dtype=torch.long
        )
        ...
        self.A1 = ssp.csr_matrix(
            (ew, (ei[0], ei[1])), shape=(self.num_nodes, self.num_nodes)
        )
        # add one more edge connecting the squares
        bridge2 = torch.tensor([[1, 3, 5, 7], [3, 1, 7, 5]])
        ei1 = torch.cat([self.edge_index, bridge2], dim=1)
        ew1 = torch.ones(ei1.size(1), dtype=int)
        self.A2 = ssp.csr_matrix(
            (ew1, (ei1[0], ei1[1])), shape=(self.num_nodes, self.num_nodes)
        )
