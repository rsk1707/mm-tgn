...
class BUDDYTests(unittest.TestCase):
    ...
    def test_get_buddy_preds(self):
        ...
        pos_pred, neg_pred, pred, labels = get_buddy_preds(
            model, test_loader, device, args=args, split="test"
        )
    ...
    def test_train_buddy(self):
        ...
        link_loader = DataLoader(
            range(len(train_dataset.links)), args.batch_size, shuffle=True
        )
        for batch_count, indices in enumerate(link_loader):
            ...
            if args.use_RA:
                ...
                RA = None
            logits = model(
                subgraph_features, node_features, degrees[:, 0], degrees[:, 1], RA
            )
        ...
        optimizer = torch.optim.Adam(
            params=parameters, lr=args.lr, weight_decay=args.weight_decay
        )
