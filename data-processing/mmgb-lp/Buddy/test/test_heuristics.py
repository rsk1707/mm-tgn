...
class HeuristicTests(unittest.TestCase):
    def setUp(self):
        ...
        square1 = np.array([[0, 1, 1, 2, 2, 3, 3, 0], [1, 0, 2, 1, 3, 2, 0, 3]])
        square2 = square1 + 4
        bridge = np.array([[0, 4], [4, 0]])

        common_neigbours = np.array(
            [[0, 9, 9, 4, 0, 8, 4, 8], [9, 0, 4, 9, 8, 0, 8, 4]]
        )

        self.ei = torch.tensor(
            np.concatenate([square1, square2, bridge, common_neigbours], axis=1),
            dtype=torch.long,
        )
        ew = torch.ones(self.ei.size(1), dtype=int)
        num_nodes = 10
        self.A1 = ssp.csr_matrix(
            (ew, (self.ei[0], self.ei[1])), shape=(num_nodes, num_nodes)
        )
    ...
    def test_iso_graph(self):
        A = ssp.csr_matrix(
            (
                self.iso_edge_weight,
                (self.iso_edge_index[:, 0], self.iso_edge_index[:, 1]),
            ),
