...
class ELPHDatasetTests(unittest.TestCase):
    ...
    def test_get_subgraph_features_batched(self):
        ...
        if os.path.exists(hash_name) and os.path.exists(cards_name):
            ...
            torch.save(cards, cards_name)
        all_edges = torch.cat([self.pos_edges, self.neg_edges], 0)
        subgraph_features = eh.get_subgraph_features(
            all_edges, hashes, cards, batch_size=batch_size
        )
        self.assertTrue(
            subgraph_features.shape
            == (len(all_edges), self.args.max_hash_hops * (self.args.max_hash_hops + 2))
        )
    ...
    def test_make_train_eval_dataset(self):
        ...
        train_eval_dataset = make_train_eval_data(
            self.args,
            hdd,
            self.n_nodes,
        ...
        self.assertTrue(
            len(train_eval_dataset.links) == (negs_per_pos + 1) * n_pos_samples
        )
        self.assertTrue(
            len(train_eval_dataset.labels) == (negs_per_pos + 1) * n_pos_samples
        )
        self.assertTrue(
            len(train_eval_dataset.subgraph_features)
            == (negs_per_pos + 1) * n_pos_samples
        )
        ...
        self.assertTrue(
            len(train_eval_dataset.RA) == (negs_per_pos + 1) * n_pos_samples
        )
