...
def load_usair_dataset(device, dataset_root):
    ...
    load_split = np.load(os.path.join(dataset_root, "USAir.npz"), allow_pickle=True)[
        "current"
    ].item()
    ...
    train_idx = torch.cat(
        (
            torch.Tensor(load_split["train_pos"][0]).unsqueeze(1),
            torch.Tensor(load_split["train_pos"][1]).unsqueeze(1),
        ),
        dim=1,
    )
    valid = torch.cat(
        (
            torch.Tensor(load_split["valid_pos"][0]).unsqueeze(1),
            torch.Tensor(load_split["valid_pos"][1]).unsqueeze(1),
        ),
        dim=1,
    )
    test = torch.cat(
        (
            torch.Tensor(load_split["test_pos"][0]).unsqueeze(1),
            torch.Tensor(load_split["test_pos"][1]).unsqueeze(1),
        ),
        dim=1,
    )
    edge_split["train"] = {}
...
def load_esci_dataset(device, dataset_root):
    ...
    reverse_eids = reverse_eids.to(device)
    seed_edges = torch.arange(g.num_edges()).to(device)
    load_split = np.load(os.path.join(dataset_root, "esci.npz"), allow_pickle=True)[
        "current"
    ].item()
...
def remove_collab_dissimilar_edges(dataset):
    ...
    new_edge_split = {
        ...
        "valid": {
            "edge": edge_split["valid"]["edge"],
            "weight": edge_split["valid"]["weight"],
            "year": edge_split["valid"]["year"],
            "edge_neg": edge_split["valid"]["edge_neg"],
        },
        "test": {
            "edge": edge_split["test"]["edge"],
            "weight": edge_split["test"]["weight"],
            "year": edge_split["test"]["year"],
            "edge_neg": edge_split["test"]["edge_neg"],
        },
    }
    for split in ["train"]:
        ...
        new_edge_split[split]["edge"] = torch.tensor(
            new_edge_split[split]["edge"], dtype=torch.int64
        )
        new_edge_split[split]["weight"] = torch.tensor(
            new_edge_split[split]["weight"], dtype=torch.int64
        )
        new_edge_split[split]["year"] = torch.tensor(
            new_edge_split[split]["year"], dtype=torch.int64
        )
    ...
    for head_node in range(num_of_nodes):
        ...
        if head_degree > 1:
            ...
            for dst_node in dst_node_list:
                simi_vector.append(
                    cosi(node_feature[head_node], node_feature[dst_node])
                )
