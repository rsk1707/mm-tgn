...
from models import GCN, MLP, SAGE, Dot, GATv2
...
def train(
    ...
    checkpoint_path = (
        ...
        + str(cfg.lr)
        + "_exclude_degree_"
        + str(cfg.exclude_target_degree)
        + "_full_neighbor_"
        + str(cfg.full_neighbor)
        + "_accu_num_"
        + str(cfg.accum_iter_number)
        + "_trail_"
        + str(run)
        + "_best.pth"
    )
    if cfg.full_neighbor:
        ...
        sampler = MultiLayerFullNeighborSampler(
            num_layers=cfg.num_layers, prefetch_node_feats=["feat"]
        )
    else:
        log.info("We sample the neighbor node of the target node to train the models. ")
        sampler = NeighborSampler(
            [cfg.num_of_neighbors] * cfg.num_layers, prefetch_node_feats=["feat"]
        )
    ...
    for epoch in range(cfg.n_epochs):
        ...
        if (epoch + 1) % cfg.log_steps == 0:
            ...
            with torch.no_grad():
                ...
                log.info(
                    "do evaluation on training examples: check if can be overfitted"
                )
                ...
                neg_dst = edge_split["eval_train"]["target_node_neg"].to(
                    node_emb.device
                )
                ...
                mrr, hits_at_10, hits_at_1 = compute_mrr_esci(
                    ...
                    neg_dst,
                    device,
                    preload_node_emb=cfg.preload_node_emb,
                ...
                if cfg.no_eval is False:
                    ...
                    for split in ["valid", "test"]:
                        if cfg.dataset == "ogbl-citation2":
                            ...
                            neg_dst = edge_split[split]["target_node_neg"].to(
                                node_emb.device
                            )
                            results.append(
                                compute_mrr(
                    ...
                    if len(logger.results[run]) > 0:
                        previous_best_valid_result = (
                            torch.tensor(logger.results[run])[:, 0].max().item()
                        )
                    else:  # length = 0
                    ...
                    log.info(
                        "Validation Hits@10 {:.4f}, Test Hits@10 {:.4f}".format(
                            results[0][1].item(), results[1][1].item()
                        )
                    )
                    log.info(
                        "Validation Hits@1 {:.4f}, Test Hits@1 {:.4f}".format(
                            results[0][2].item(), results[1][2].item()
                        )
                    )
...
def main(cfg: DictConfig):
    ...
    verbose = True
    device = (
        "cuda" if cfg.mode == "puregpu" else "cpu"
    ...
    for run in range(cfg.runs):
        ...
        mrr, h10, h1 = train(
            cfg, device, g, reverse_eids, seed_edges, model, edge_split, logger, run
        )
        mrrs.append(mrr)
        h10s.append(h10)
        h1s.append(h1)
