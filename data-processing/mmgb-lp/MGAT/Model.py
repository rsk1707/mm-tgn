...
class MGAT(torch.nn.Module):
    def __init__(
        self, features, edge_index, batch_size, num_nodes, num_layers: int, dim_x=64
    ):
    ...
    def forward(self, user_nodes, pos_items, neg_items):
        ...
        if a_rep is None:
            ...
            representation = (
                (v_rep + a_rep + t_rep) / 3
            )  # torch.max_pool2d((v_rep, a_rep, t_rep))#max()#torch.cat((v_rep, a_rep, t_rep), dim=1)
    ...
    def loss(self, data):
        ...
        pos_scores, neg_scores = self.forward(
            user.cuda(), pos_items.cuda(), neg_items.cuda()
        )
...
class GNN(torch.nn.Module):
    ...
    def forward(self, id_embedding):
        temp_features = (
            torch.tanh(self.MLP(self.features)) if self.dim_latent else self.features
        )
        x = temp_features
