...
class LightGCN(object):
    ...
    def _create_gcmc_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            # dense layer.
            mlp_embeddings = tf.matmul(embeddings, self.weights['W_mlp_%d' % k]) + \
...
class train_thread(threading.Thread):
    ...
    def run(self):
        ...
        self.data = sess.run(
            [self.model.opt, self.model.loss, self.model.mf_loss,
             self.model.emb_loss, self.model.reg_loss],
...
if __name__ == '__main__':
    ...
    if args.pretrain == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                pretrain_ret = 'pretrained model hit=[%s], ndcg=[%s], mrr=[%s]' % (
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        f.write(
            ...
            % (args.embed_size, args.lr, args.layer_size, args.keep_prob,
               args.regs, args.loss_type, args.adj_type)
        )

        for i, users_to_test in enumerate(users_to_test_list):
            ret = test(sess, model, users_to_test, drop_flag=True)

            final_perf = "hit=[%s], ndcg=[%s], mrr=[%s]" % \
                         (', '.join(['%.5f' % r for r in ret['hit']]),
                          ', '.join(['%.5f' % r for r in ret['ndcg']]),
                          ', '.join(['%.5f' % r for r in ret['mrr']]))
    ...
    *********************************************************
    ...
    train_writer = tf.summary.FileWriter(
        tensorboard_model_path + model.log_dir + '/run_' + str(run_time),
        sess.graph
    )
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        summary_train_loss = sess.run(
            model.merged_train_loss,
            feed_dict={model.train_loss: loss,
                       model.train_mf_loss: mf_loss,
                       model.train_emb_loss: emb_loss,
                       model.train_reg_loss: reg_loss}
        )
        ...
        perf_str = 'Epoch %d: train==[%.5f=%.5f + %.5f + %.5f], hit=[%s], ndcg=[%s], mrr=[%s]' % \
                   (epoch, loss, mf_loss, emb_loss, reg_loss,
                    ...
                    ', '.join(['%.5f' % r for r in ret['mrr']]))
        print(perf_str)
        summary_train_acc = sess.run(
            model.merged_train_acc,
            feed_dict={model.train_rec_first: ret['hit'][0],
        ...
        # parallelized sampling for test loss
        loss_test = loss
        mf_loss_test = mf_loss
        emb_loss_test = emb_loss
        reg_loss_test = reg_loss

        summary_test_loss = sess.run(
            model.merged_test_loss,
            feed_dict={
                model.test_loss: loss_test,
                model.test_mf_loss: mf_loss_test,
                model.test_emb_loss: emb_loss_test,
                model.test_reg_loss: reg_loss_test
            }
        )
        ...
        pre_loger.append(ret['mrr'])
        ndcg_loger.append(ret['ndcg'])

        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: test==[%.5f=%.5f + %.5f + %.5f], hit=[%s], ndcg=[%s], mrr=[%s]' % \
        ...
        cur_best_pre_0, stopping_step, should_stop = early_stopping(
            ret['hit'][0], cur_best_pre_0, stopping_step,
    ...
    recs = np.array(rec_loger)     # hit@K
    pres = np.array(pre_loger)     # mrr@K
    ndcgs = np.array(ndcg_loger)   # ndcg@K

    best_hit_0 = max(recs[:, 0])
    idx = list(recs[:, 0]).index(best_hit_0)

    final_perf = "Best Iter=[%d]@[%.1f]\thit=[%s], mrr=[%s], ndcg=[%s]" % \
                 (idx, time() - t0,
    ...
    f.write(
        ...
        % (args.embed_size, args.lr, args.layer_size, args.node_dropout,
